...
class ResourceTestCases(ToDoTestBase):
    ...
    def test_all_api_endpoints_without_token(self):
        # get list of todos
        response = self.app.get(
            '/api/v1/todos', headers={'Content-Type': 'application/json'})
        self.assertEqual(response.status_code, 401)
        # add todo item
        todo = json.dumps(
            {"name": "do that thing", "created_by": 1, "edited": False})
        response = self.app.post(
            '/api/v1/todos',
            data=todo,
        ...
        response = self.app.get(
            '/api/v1/todos/1', headers={'Content-Type': 'application/json'})
        self.assertEqual(response.status_code, 401)
...
if __name__ == '__main__':
    try:
        if config.TESTING is not True:
            raise Exception(
                "Config.TESTING is not True. To allow testing set "
                "config.TESTING to True to create a testing database and "
                "ensure main database is not used.")
    except Exception as err:
        print(err)
    else:
        unittest.main()
